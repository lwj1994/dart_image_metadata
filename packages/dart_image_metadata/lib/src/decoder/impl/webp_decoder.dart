  ...
  Future<ImageMetadata> parse(ImageInput input) async {
    try {
      ...
      if (_isExtendedFormat(chunkHeader)) {
        ...
        final heightList = await input.getRange(0x1b, 0x1e);
        ...
        final bits = sizeList
            .expand((i) => i.toRadixString(2).padLeft(8, '0').split(''))
            .toList();
        width = (int.tryParse(bits.sublist(0, 14).join(), radix: 2) ?? 0) + 1;
        height = (int.tryParse(bits.sublist(14, 28).join(), radix: 2) ?? 0) + 1;
      } else {
